"""Upload local UTF-8 text files to Cloudflare R2.

For each *.utf8.txt in the local txt-dir:
  1. Compare MD5 of the local file against the ETag of the existing R2 object.
  2. If the content differs (or is missing with --upload-new), upload it.

Usage:
    uv run python scripts/migrate_txt_r2.py [--dry-run] [--upload-new] [--txt-dir DIR] [--concurrency N]
"""
...
class Stats:
    changed: int = 0
    unchanged: int = 0
    missing: int = 0  # no TXT in R2 — skipped intentionally
    errors: int = 0
...
def process_file(
    ...
    """Upload one UTF-8 text file to R2 if it is new or changed.
    ...
    try:
        ...
        logger.error("Read error %s: %s", txt_path.name, e)
        return "error"

    new_md5 = hashlib.md5(content).hexdigest()
    try:
        head = s3.head_object(Bucket=bucket, Key=txt_key)
        ...
        if e.response["Error"]["Code"] in ("NoSuchKey", "404", "403"):
            if upload_new:
                ...
                logger.debug("Skipping %s — no TXT in R2", txt_key)
                return "missing"
        else:
            logger.error("R2 head error %s: %s", txt_key, e)
            return "error"

    if outcome == "changed":
        if dry_run:
            logger.info("[dry-run] would upload %s", txt_key)
            return outcome
        try:
            s3.put_object(
                Bucket=bucket,
            ...
            logger.info("Uploaded %s", txt_key)
        except Exception as e:
            logger.error("Upload error %s: %s", txt_key, e)
            return "error"
...
def main() -> None:
    ...
    parser.add_argument(
        "--upload-new",
        action="store_true",
        help="Upload files that have no existing TXT in R2 (new books)",
    )
    parser.add_argument(
        "--concurrency",
        type=int,
